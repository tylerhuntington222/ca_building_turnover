...
output.metalist <- foreach(shapefile = in.shapes[1:length(in.shapes)],
  ...
  for (st in subtype.iterators) {
   ...
   if (fs.diff < 0) {
     print("Floorspace difference is negative")
     print(sprintf("Randomly vacating remaining buildings of subtype: %s", st))
     # randomly vacate remaining buildings until diff is positive
     st.diff <- fs.diff
     while (st.diff < 0) {
       res <- RandomSwitch(cnt.raw.df, from.stype = st, from.status = "Remain",
  ...
  # case 2: floorspace difference is positive
     ...
     if (ratio < 1) {
       ...
       # init floorspace difference tracker for this usetype
       st.diff <- CheckDiff(cnt.raw.df, t, status = c("Remain", "Rebuild"),
                           year = 2050, area.col="BArea.50")
